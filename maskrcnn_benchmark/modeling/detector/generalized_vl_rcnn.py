...
from maskrcnn_benchmark.modeling.relation_head import build_relation_head
...
class GeneralizedVLRCNN(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        if cfg.MODEL.LANGUAGE_BACKBONE.TOKENIZER_TYPE == "clip":
            ...
            # self.tokenizer = AutoTokenizer.from_pretrained(cfg.MODEL.LANGUAGE_BACKBONE.TOKENIZER_TYPE, local_files_only=cfg.MODEL.LANGUAGE_BACKBONE.TOKENIZER_LOCAL_FILES_ONLY)
            self.tokenizer = AutoTokenizer.from_pretrained(cfg.MODEL.LANGUAGE_BACKBONE.MODEL_DIR)
    ...
    def forward(self, 
        ...
        if self.cfg.MODEL.RELATION_ON:
            for p in result: p.add_field("pred_labels", p.get_field("labels").clone()) # initial label
            x, detections, loss_relation = self.relation_head(x, result, targets)
        if self.training:
            losses = {}
