...
from ..relation_head.ov_classifier import CLIPDynamicClassifierSimple, BLIPDynamicClassifierSimple, union_pixel, Adapter, sigmoid_focal_loss, accuracy, standardization, CLIPDynamicClassifierBaseline, PadPrompter, PatchPrompter
import clip
...
class VLDyHead(torch.nn.Module):
    def __init__(self, cfg):
        ...
        if cfg.MODEL.LANGUAGE_BACKBONE.MODEL_TYPE == "bert-base-uncased":
            ...
            lang_cfg = BertConfig.from_pretrained(cfg.MODEL.LANGUAGE_BACKBONE.MODEL_DIR)
...
class VLDyHeadModule(torch.nn.Module):
    ...
    def __init__(self, cfg):
        ...
        # open-vocabulary predicate classifier
        if cfg.MODEL.DYHEAD.OV.ENABLED:
            self.clip_model = None
            self.clip_preprocess = None
            self.clip_dynamic_classifier = None
            self.clip_input_dim = 256
            if self.cfg.MODEL.DYHEAD.OV.CLIP_FEAT_DISTILLATION or self.cfg.MODEL.DYHEAD.OV.DYNAMIC_CLIP_CLASSIFIER:
                if self.cfg.MODEL.DYHEAD.OV.FEAT_DISTILLATION_MODEL == 'clip':
                    clip_hidden_dim = 512  # the clip embed_dim
                    self.clip_linear = nn.Sequential(
                        nn.Linear(self.clip_input_dim, clip_hidden_dim),
                        nn.LayerNorm(clip_hidden_dim),
                    )
                    ...
                    # add adapter layer
                    if self.cfg.MODEL.DYHEAD.OV.USE_ADAPTER_LAYER:
                        self.clip_adapter = Adapter(512, 2)
                        for param in self.clip_adapter.parameters():
                            param.requires_grad = False
                else:
                    clip_hidden_dim = 256  # the clip embed_dim
                    self.clip_linear = nn.Sequential(
                        nn.Linear(self.clip_input_dim, clip_hidden_dim),
                        nn.LayerNorm(clip_hidden_dim),
                    )
                    ...
                    self.blip_model, self.vis_preprocess, self.txt_processors = load_model_and_preprocess(name="blip2_feature_extractor", model_type="pretrain", is_eval=True, device=device)
                    for param in self.blip_model.parameters():
                        param.requires_grad = False
                    self.txt_processors["eval"].max_words = 108

            if self.cfg.MODEL.DYHEAD.OV.DYNAMIC_CLIP_CLASSIFIER:
                if self.cfg.MODEL.DYHEAD.OV.FEAT_DISTILLATION_MODEL == 'clip':
                    if self.cfg.MODEL.DYHEAD.OV.DYNAMIC_CLIP_CLASSIFIER:
                        self.clip_dynamic_classifier = CLIPDynamicClassifierSimple(
                            cfg, input_dim=self.clip_input_dim, clip_model=self.clip_model,
                            clip_preprocess=self.clip_preprocess
                        )
                    else:
                        self.clip_dynamic_classifier = CLIPDynamicClassifierBaseline(
                            self.cfg, input_dim=self.clip_input_dim, clip_model=self.clip_model,
                            clip_preprocess=self.clip_preprocess
                        )
                else:
                    self.clip_dynamic_classifier = BLIPDynamicClassifierSimple(
                        cfg, input_dim=self.clip_input_dim, clip_model=self.blip_model,
                        clip_preprocess=self.txt_processors
                    )

                self.rel_class_embed = None
            else:
                if self.cfg.MODEL.DYHEAD.OV.CLIP_FEAT_DISTILLATION:
                    self.rel_class_embed = nn.Linear(self.clip_input_dim, self.cfg.MODEL.ROI_RELATION_HEAD.NUM_CLASSES)
                else:
                    self.rel_class_embed = nn.Linear(cfg.MODEL.DYHEAD.FUSE_CONFIG.JOINT_EMB_SIZE, self.cfg.MODEL.ROI_RELATION_HEAD.NUM_CLASSES)

                if self.cfg.MODEL.DYHEAD.OV.ENABLED:
                    seen_cls_num = self.cfg.MODEL.ROI_RELATION_HEAD.NUM_CLASSES - len(self.cfg.MODEL.DYHEAD.OV.ZS_PREDICATES)
                    if self.cfg.MODEL.DYHEAD.OV.CLIP_FEAT_DISTILLATION:
                        self.rel_class_embed_train = nn.Linear(self.clip_input_dim, seen_cls_num)
                    else:
                        self.rel_class_embed_train = nn.Linear(cfg.MODEL.DYHEAD.FUSE_CONFIG.JOINT_EMB_SIZE, seen_cls_num)
        self.result = {}
    ...
    def _forward_train(self, box_cls, box_regression, centerness, targets, anchors,
        ...
        if self.cfg.MODEL.DYHEAD.RELATION_CONSISTENCY_ON:
            ...
            all_union_boxes, all_rel_tgt_labels_one_hot, num_rels = [], [], []
            for img_id, target in enumerate(targets):
                ...
                # if use focal loss, change label to one hot label
                if self.cfg.MODEL.DYHEAD.OV.RELATION_LOSS == 'focal':
                    if self.cfg.MODEL.DYHEAD.OV.ENABLED:
                        num_classes = self.cfg.MODEL.ROI_RELATION_HEAD.NUM_CLASSES - len(self.cfg.MODEL.DYHEAD.OV.ZS_PREDICATES)
                    else:
                        num_classes = self.cfg.MODEL.ROI_RELATION_HEAD.NUM_CLASSES
                    y_onehot = F.one_hot(sampled_rel_labels, num_classes=num_classes)[:,1:]
                    all_rel_tgt_labels_one_hot.append(y_onehot)
                
                # collect reps & tgts
                all_pair_reps.append(pair_reps)
                num_rels.append(pair_reps.shape[0])
            ...
            # if use focal loss, change label to one hot label
            if self.cfg.MODEL.DYHEAD.OV.RELATION_LOSS == 'focal':
                all_rel_tgt_labels_one_hot = torch.cat(all_rel_tgt_labels_one_hot, dim=0)

            if self.cfg.MODEL.DYHEAD.OV.ENABLED:
                if all_pair_reps_refine.size(0) == 0:
                    rel_cls_logits = torch.zeros((all_rel_tgt_labels.shape[0], 36)).to(all_pair_reps_refine.device)
                    rel_cls_logits = rel_cls_logits.type(torch.float32)
                    losses['rel_clip_distill_loss'] = torch.tensor(0.0, requires_grad=True)
                else:
                    ...
                    if self.cfg.MODEL.DYHEAD.OV.FEAT_DISTILLATION_MODEL == 'clip':
                        ...
                        target_clip_inputs = torch.cat(proc_imgs).to(all_pair_reps.device)
                        with torch.no_grad():
                    ...
                    if self.clip_dynamic_classifier is not None:
                        ...
                        if self.cfg.MODEL.DYHEAD.OV.CLIP_FEAT_DISTILLATION:
                            target_clip_feats = all_pair_reps_refine
                            rel_clip_hs = self.clip_linear(all_pair_reps_refine)
                        rel_cls_logits = self.rel_class_embed_train(rel_clip_hs)  # # num_pair, 51-ZS
                    ...
                    if self.cfg.MODEL.DYHEAD.OV.CLIP_FEAT_DISTILLATION:
                        ...
                        for rel_clip_hs_split_i in rel_clip_hs_split:
                            rel_clip_hs_distillation.append(torch.mean(rel_clip_hs_split_i, dim=0))
                        rel_clip_hs_distillation = torch.stack(rel_clip_hs_distillation).to(rel_clip_hs.device) 
                        losses['rel_clip_distill_loss'] = F.l1_loss(rel_clip_hs_distillation, target_clip_feats) * self.cfg.MODEL.DYHEAD.OV.CLIP_FEAT_DISTILLATION_LOSS_COEFF

            else:
                rel_cls_logits = self.relation_semantic_embed(all_pair_reps_refine)  # shape: num_pair x 51
            ...
            # use ce loss or focal loss
            if self.cfg.MODEL.DYHEAD.OV.RELATION_LOSS == 'focal':
                alpha = 0.25
                gamma = 2.0
                all_rel_tgt_labels_one_hot = all_rel_tgt_labels_one_hot.float()
                num_gt_rel = all_rel_tgt_labels_one_hot.shape[0]
                num_gt_rel = torch.as_tensor(
                    [num_gt_rel], dtype=torch.float, device=all_rel_tgt_labels_one_hot.device
                )
                if rel_cls_logits.size() == all_rel_tgt_labels_one_hot.size():
                    loss_rel_focal = (
                        sigmoid_focal_loss(
                            rel_cls_logits,
                            all_rel_tgt_labels_one_hot,
                            num_gt_rel,
                            alpha=alpha,
                            gamma=gamma,
                        )
                        * all_rel_tgt_labels_one_hot.shape[0]
                    )
                    losses['rel_semantic_cons_loss'] = loss_rel_focal[0]
                else:
                    losses['rel_semantic_cons_loss'] = torch.tensor(0.0, requires_grad=True).to(rel_cls_logits.device)
            else:
    ...
    def _forward_test(self, box_regression, centerness, anchors,
        ...
        if self.cfg.MODEL.DYHEAD.RELATION_CONSISTENCY_ON:
            ...
            for img_id, boxes_per_img in enumerate(boxes):
                ...
                # use ce loss or focal loss
                if self.cfg.MODEL.DYHEAD.OV.RELATION_LOSS == 'focal':
                    rel_class_prob = torch.sigmoid(relation_logits)
                    topk, cls_num = rel_class_prob.shape
                    topk_values_all, topk_indexes_all = torch.sort(
                        rel_class_prob.reshape(-1), dim=-1, descending=True
                    )  # num_pair * num_cls
                    rel_scores = topk_values_all[:topk]
                    total_pred_idx = torch.div(topk_indexes_all[:topk], cls_num, rounding_mode='trunc')
                    rel_class = topk_indexes_all[:topk] % cls_num
                    rel_class += 1

                    ent_scores = obj_scores[pair_ids].prod(-1)[total_pred_idx]
                    triple_scores = rel_scores * ent_scores
                    _, sorting_idx = torch.sort(triple_scores.view(-1), dim=0, descending=True)
                    rel_pair_idx = pair_ids[total_pred_idx][sorting_idx]
                    rel_class_prob = rel_class_prob[total_pred_idx][sorting_idx] # 50, Num_rel
                    zeros = torch.zeros(rel_class_prob.size(0), 1).to(rel_class_prob.device)
                    rel_class_prob_expanded = torch.cat((zeros, rel_class_prob), dim=1)
