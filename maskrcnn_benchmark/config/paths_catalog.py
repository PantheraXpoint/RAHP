...
class DatasetCatalog(object):
    DATASETS = {
        ...
        # vg150 for SGG
        "vg150": {
            "img_dir": "VG150/VG_100K",
            "roidb_file": "VG150/VG-SGG-with-attri.h5",
            "dict_file": "VG150/VG-SGG-dicts-with-attri.json",
            "image_file": "VG150/image_data.json"
        },
        # language-supervised SGG
        "cococaption_scene_graph": {
            "img_dir": "coco/train2017",
            "img_meta_info_file": 'coco/annotations/captions_train2017.json',
            "caption_scene_graph_file": "VG150/weak_supervisions/language_supervised-coco_caption-grounding=glipL-javaparser.json"
        },
        "vgcaption_scene_graph": {
            "img_dir": "VG150/VG_100K",
            "img_meta_info_file": "VG150/image_data.json",
            "caption_scene_graph_file": "VG150/weak_supervisions/language_supervised-vg150train-grounding=glipL.json"
        },
        # unbounded SGG
        "unbounded_vg_scene_graph": {
            "img_dir": "VG150/VG_100K",
            "img_meta_info_file": "VG150/image_data.json",
            "scene_graph_file": "VG150/unbounded_SGG/scene_graph_VG_v14.json"
        },
    }
    ...
    def get(name, cfg=None):
        ...
        if name.endswith('_bg'):
            ...
            if "bing" in name.split("_"):
                ...
                args['filter_empty_rels'] = cfg.MODEL.RELATION_ON or cfg.MODEL.DYHEAD.RELATION_CONSISTENCY_ON
                args['flip_aug'] = cfg.MODEL.FLIP_AUG
                args['split_key'] = cfg.DATASETS.VG150_SPLIT_KEY
                args['box_grounding_file'] = cfg.DATASETS.VG150_GRAPH_GROUNDING_FILE
                ...
                """add open-predicate"""
                args['open_predicate_mode'] = cfg.MODEL.DYHEAD.OV.ENABLED
                args['open_predicate_category'] = cfg.MODEL.DYHEAD.OV.ZS_PREDICATES
                return dict(
                    factory="VG150Dataset",
                    args=args,
                )
            # language-supervised SGG
                ...
                args['open_vocabulary_mode'] = cfg.DATASETS.VG150_OPEN_VOCAB_MODE
                return dict(
                    factory="COCOCaptionSceneGraphDataset",
                    args=args,
                )
            elif 'vgcaption_scene_graph' in name:
                args = copy.deepcopy(DatasetCatalog.DATASETS[name])
                data_dir = try_to_find(args["img_dir"], return_dir=True)
                for k, v in args.items():
                    args[k] = os.path.join(data_dir, v)
            ...
            # Unbounded SGG
                ...
                args['train_text_input_type'] = cfg.MODEL.DYHEAD.RELATION_TRAIN_TEXT_INPUT_TYPE
                return dict(
                    factory="UnboundedVGSceneGraphDataset",
                    args=args,
                )
            else:
                attrs = DatasetCatalog.DATASETS[name]
