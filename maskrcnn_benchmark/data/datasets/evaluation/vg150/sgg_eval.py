...
class SGZeroShotRecall(SceneGraphEvaluation):
    def __init__(self, result_dict, unseen_obj_cats, unseen_predicates):
        super(SGZeroShotRecall, self).__init__(result_dict)
        self.novel_count = 0
        self.all_count = 0
        
        # Store the unseen categories passed from config
        self.unseen_obj_cats = unseen_obj_cats
        self.unseen_predicate_cars = set(unseen_predicates)
        
        # Load base pair labels tensor and convert to set of tuples
        import torch
        base_pairs_tensor = torch.load(
            "/RAHP/DATA/vg/base_pair_labels_all.pt",
            map_location=torch.device("cpu"),
            weights_only=False  # Suppress the warning
        )
        
        # Convert tensor of shape [N, 2] to set of tuples for fast membership testing
        self.base_pair_labels_all = set(
            (int(pair[0]), int(pair[1])) for pair in base_pairs_tensor
        )
        
        print(f"✅ SGZeroShotRecall: Loaded {len(self.base_pair_labels_all)} base pairs")
        print(f"✅ Using {len(self.unseen_obj_cats)} unseen object categories")
        print(f"✅ Using {len(self.unseen_predicate_cars)} unseen predicates")
