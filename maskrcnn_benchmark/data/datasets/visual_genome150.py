...
"""open-predicate split"""
"""cacao split"""
# SEEN_PREDICATES = [0, 1, 3, 6, 7, 8, 9, 10, 11, 12, 13, 16, 17, 19, 20, 21, 22, 23, 25, 27, 29, 30, 31, 33, 35, 37, 38, 40, 41, 42, 43, 46, 47, 48, 49, 50]
# ZS_PREDICATES = [2, 4, 5, 14, 15, 18, 24, 26, 28, 32, 34, 36, 39, 44, 45]
"""OvSGTR split"""
# SEEN_PREDICATES = [0, 1, 2, 3, 4, 5, 6, 8, 10, 11, 12, 13, 15, 17, 18, 19, 20, 21, 23, 24, 25, 26, 27, 28, 29, 32, 33, 35, 40, 41, 42, 43, 44, 47, 48, 50]
# ZS_PREDICATES = [7, 9, 14, 16, 22, 30, 31, 34, 36, 37, 38, 39, 45, 46, 49]
...
class VG150Dataset(torch.utils.data.Dataset):
    ...
    def __init__(self, split, img_dir, roidb_file, dict_file, image_file, transforms=None,
                ...
                filter_duplicate_rels=True, filter_non_overlap=True, flip_aug=False, box_grounding_file=None, open_vocabulary_mode=False, open_predicate_mode=False, open_predicate_category=None):
        """
        ...
        self.open_predicate_mode = open_predicate_mode
        self.unseen_predicate_category = open_predicate_category
        self.seen_predicate_category = [i for i in range(51) if i not in open_predicate_category]
        ...
        """add open-predicate"""
        if self.open_predicate_mode:
            self.seen_class = []
            self.unseen_class = self.unseen_predicate_category
            for cate_i in range(len(self.ind_to_predicates)):
                if cate_i not in self.unseen_class:
...
def load_graphs(roidb_file, split, num_im, num_val_im, filter_empty_rels, filter_non_overlap, split_key='split', keep_base_classes=False, keep_base_predicates=False):
    ...
    for i in range(len(image_index)):
        ...
        """
            for split unseen predicate
        """
        if keep_base_predicates:
            assert split == 'train'
            seen_rels = []
            for r in rels:
                if split == 'train':
            ...
            if len(seen_rels) == 0:
                ...
                rels = np.array(seen_rels)
        """
            add for split unseen predicate
        """
