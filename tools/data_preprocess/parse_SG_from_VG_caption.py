...
parser.add_argument("--caption_from", type=str, default='VG')
...
# parse triplets from captions
processed_vg_caption_file = 'DATASET/VG150/weak_supervisions/language_supervised-vg150train.json'

if os.path.exists(processed_vg_caption_file):
    with open(processed_vg_caption_file, 'r', encoding='utf-8') as fin:
        vg_parsing_info = json.load(fin)
else:
    with open('DATASET/VG150/region_descriptions.json', 'r') as fin:
        vg_captions = json.load(fin)
    vg_imgid2region_captions = {x['id']: x['regions'] for x in vg_captions}

    # parsed caption scene graphs
    vg_imgid2triplets = {}
    caption_object_vocabs, caption_relation_vocabs = set(), set()
    for imgid in tqdm(img_ids_in_vg_train):
        image_triplets = set()
        img_regions = vg_imgid2region_captions[imgid]
        for cap in img_regions:
            cap_graph = sng_parser.parse(cap['phrase'].lower()) # text parsing
            ...
            for r in cap_graph['relations']:
                ...
                image_triplets.add(triplet)
        vg_imgid2triplets[imgid] = list(image_triplets)
    ...
    caption_object_vocabs_to_vg_objs, caption_relation_vocabs_to_vg_rels = map_caption_concepts_to_vg150_categories(
        list(caption_object_vocabs), list(caption_relation_vocabs), vg_dataset
    )

    # convert to and only keep VG150 valid triplets
    image_sg_infos = {}
    for imgid, triplets in vg_imgid2triplets.items():
    ...
    with open(processed_vg_caption_file, 'w', encoding='utf-8') as fout:
        json.dump(vg_parsing_info, fout, ensure_ascii=False, indent=4)

print(f"image_count/no_triplet_image_count = {len(vg_parsing_info['text_scene_graph'])}/{len([k for k,v in vg_parsing_info['text_scene_graph'].items() if len(v['triplets'])==0])}")
print(f"triplet_count = {sum([len(v['triplets']) for k,v in vg_parsing_info['text_scene_graph'].items()])}")
print(f"caption_object_vocabs_to_vg_objs mapped/all = {len([v for k,v in vg_parsing_info['caption_object_vocabs_to_vg_objs'].items() if len(v)>0])}/{len(vg_parsing_info['caption_object_vocabs_to_vg_objs'])}")
print(f"caption_relation_vocabs_to_vg_rels mapped/all = {len([v for k,v in vg_parsing_info['caption_relation_vocabs_to_vg_rels'].items() if len(v)>0])}/{len(vg_parsing_info['caption_relation_vocabs_to_vg_rels'])}\n\n")


########################### obtain grounded boxes of entities ###########################
if 'sg_grounding_infos' not in vg_parsing_info:
    glip_demo = GLIPDemo(
        cfg,
        min_image_size=800,
    ...
    for index in tqdm(range(len(vg_dataset))):
        ...
        img_id = vg_dataset.img_info[index]['image_id']
        ...
        img_sg_info = vg_parsing_info['text_scene_graph'][img_id] if img_id in vg_parsing_info['text_scene_graph'] else vg_parsing_info['text_scene_graph'][str(img_id)]
        img_triplets = []
        for (s, p, o) in img_sg_info['triplets']:
            ...
            img_triplets.append({
                ...
                "vg150_predicate_label": vg_dataset.ind_to_predicates.index(p),
                "text": (s, p, o)
            })
        ...
        visual_boxes_ent_ids = box_predictions.get_field('labels')-1
        ...
        for ent_id in visual_boxes_ent_ids.unique(): # grounded box
            ...
            ent_boxes[ent_id] = ent_box # ent_id = label_id-1
        ...
        for ent_id in range(len(entities)):
            ...
            img_entities.append({
                ...
                "vg150_obj_label": vg_dataset.ind_to_classes.index(label_name),
    ...
    vg_parsing_info['sg_grounding_infos'] = {
        'all_sg_box_count': all_sg_box_count,
        'grounded_sg_box_count': grounded_sg_box_count,
        'grounded_img_ids': grounded_img_ids
    }
    with open(f"{processed_vg_caption_file.replace('.json', '')}_{args.gpu_size}-{args.local_rank}.json", 'w', encoding='utf-8') as fout:
        json.dump(vg_parsing_info, fout, ensure_ascii=False, indent=4)

sg_grounding_infos = vg_parsing_info['sg_grounding_infos']
